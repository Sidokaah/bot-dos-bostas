...
module.exports = {
    ...
    run: async (client, message, args) => {
      ...
      if(args[0] == 'nikes') {
        ...
        client.db.subtract(`nikes_${message.guild.id}_${user.id}`, 1)
        let Embed3 = new Discord.MessageEmbed()
          .setColor("RANDOM")
          .setDescription(`<:tick:748569437589995731> Vendeste Nikes por 4000 moedas!`);
        client.db.add(`money_${message.guild.id}_${user.id}`, 4000)
        ...
        client.db.add(`money_${message.guild.id}_${user.id}`, 100000)
        message.channel.send(Embed3)
        ...
        client.db.add(`money_${message.guild.id}_${user.id}`, 1750000)
        message.channel.send(Embed3)
      } else if(args[0] == 'dragonlore') {
          let Embed2 = new Discord.MessageEmbed()
              .setColor("RANDOM")
              .setDescription(`<:X:748632517476745226> Não tens nenhuma AWP Dragon Lore para vender!`);
          let houses = await client.db.fetch(`dragon_${message.guild.id}_${user.id}`)
          if (houses < 1) return message.channel.send(Embed2)
          client.db.fetch(`dragon_${message.guild.id}_${user.id}`)
          client.db.subtract(`dragon_${message.guild.id}_${user.id}`, 1)
          let Embed3 = new Discord.MessageEmbed()
              .setColor("RANDOM")
              .setDescription(`<:tick:748569437589995731> Vendeste uma AWP Dragon Lore por 40000 moedas!`);
          client.db.add(`money_${message.guild.id}_${user.id}`, 40000)
          message.channel.send(Embed3)
      } else if(args[0] == 'pcgamer') {
          let Embed2 = new Discord.MessageEmbed()
              .setColor("RANDOM")
              .setDescription(`<:X:748632517476745226> Não tens nenhuma mansão para vender!`);
          let houses = await client.db.fetch(`pc_${message.guild.id}_${user.id}`)
          if (houses < 1) return message.channel.send(Embed2)
          client.db.fetch(`pc_${message.guild.id}_${user.id}`)
          client.db.subtract(`pc_${message.guild.id}_${user.id}`, 1)
          let Embed3 = new Discord.MessageEmbed()
              .setColor("RANDOM")
              .setDescription(`<:tick:748569437589995731> Vendeste um PC Gamer por 35000 moedas!`);
          client.db.add(`money_${message.guild.id}_${user.id}`, 35000)
          message.channel.send(Embed3)
      }
          ...
          let houses = await client.db.fetch(`team_${message.guild.id}_${user.id}`)
          if (houses < 1) return message.channel.send(Embed2)
          client.db.fetch(`team_${message.guild.id}_${user.id}`)
          client.db.subtract(`team_${message.guild.id}_${user.id}`, 1)
          let Embed3 = new Discord.MessageEmbed()
              .setColor("RANDOM")
              .setDescription(`<:tick:748569437589995731> Vendeste um Unusual Burning Flames Team Captain por 80000 moedas!`);
          client.db.add(`money_${message.guild.id}_${user.id}`, 80000)
          message.channel.send(Embed3)
      }
    }
}